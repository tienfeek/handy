...
import com.tienfeek.handy.R;

/**
 * 可以下拉刷新 上拉加载更多的listview改进版 在下拉或者上拉后台数据完毕之后会自动调用adapter.notifydatasetChanged的方法
 * setEnableInterceptTouchEvent 表示会分发给子view的横向滑动手势 setBottomHasMore 底部是否存在更多
 * 
 * @author wangbin
 * 
 */
public class MxxRefreshableListView extends RefreshableListView {
    ...
    private void initRefreshListener() {
        setOnUpdateTask(new OnUpdateTask() {
            ...
            public void onUpdateStart() {
                // isNetworkAvailable =
                // MxxNetworkUtil.isNetworkAvailable(getContext());
    ...
    private void addPullDownRefreshFeature(final Context context) {
        ...
        final ProgressBar progressWheel = (ProgressBar) mListHeaderView
            .findViewById(R.id.mxx_refresh_listview_header_progresswheel);
        final ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(progressWheel, "rotationY",
            0f, 360f);// 3DX轴上（即上下轴线）0度到180度旋转
...
//                        R.string.mxx_view_string_nonetwork));// "无网络连接"
    ...
    private void addPullUpRefreshFeature(final Context context) {
        ...
        final ProgressBar progressWheel = (ProgressBar) mListBottomView
            .findViewById(R.id.mxx_refresh_listview_header_progresswheel);
        final ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(progressWheel, "rotationY",
            0f, 360f);// 3DX轴上（即上下轴线）0度到180度旋转
        objectAnimator.setDuration(animation_duration);
...
//                    infoTextView.setText(getResources().getString(
//                        R.string.mxx_view_string_nonetwork));
                    progressWheel.setVisibility(View.GONE);
                }
            }
